...
impl BareKitWorklet {
    fn init<R: Runtime>(
        ...
        let options: bare_worklet_options_t = bare_worklet_options_s {
            ...
            assets: match assets {
                Some(assets) => assets.as_ptr() as *const i8,
                None => null_mut(),
            },
        };
    ...
    fn start(&mut self, filename: String, source: String, args: Vec<String>) {
        ...
        let mut argv: Vec<*const i8> = args.iter().map(|s| s.as_ptr() as *const i8).collect();
        let err = unsafe {
            bare_worklet_start(
                self.worklet,
